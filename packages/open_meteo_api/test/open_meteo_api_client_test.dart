...
void main() {
  group('OpenMeteoApiClient', () {
    ...
    /// Before each
    setUp(() {
      httpClient = MockHttpClient(); // We're mocking the dependency of our SUT
    ...
    group('locationSearch', () {
      ...
      test('throws LocationRequestFailure non-200 response', () async {
        final response = MockResponse();
    ...
    group('getWeather', () {
      ...
      test('throws WeatherRequestFailureon non-200 response', () async {
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(400);
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        expect(
